...
.PARAMETER StaticOnly
    ...
    実行する。これらはレジストリや %LOCALAPPDATA%\...\Fonts、実インストール済みの
    scoop app に触れないマシン非依存のスイートで、'Static' タグを付けてある。
    Lifecycle / Collision / Update / RegName / FontNotify / GdiRefCount と
    Win11Debloat の実機スイートはこのスイッチでは実行しない。これらは実機を書き換える:
