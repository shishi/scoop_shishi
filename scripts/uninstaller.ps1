...
if ($unresolved.Count -eq 0) {
    ...
    #
    # 一度で諦めない。Windows では、直前に書いたファイルを含むディレクトリの
    # 再帰削除がウイルス対策やインデクサの掴みで一時的に失敗する。実際、
    # 再起動直後のテスト実行で biz-udgothic の退避削除がここで失敗し、
    # uninstall 全体が throw して以降のスイートが連鎖的に壊れ、
    # フォント環境が 127 ファイル → 40 ファイルまで破壊された。
    # 同じ操作を手で実行すると成功するので、原因は状態ではなくタイミング。
    #
    # 失敗したときは理由も出す。-ErrorAction SilentlyContinue で握りつぶすと
    # 「削除できなかった」しか分からず、次に同じことが起きても切り分けられない
    $lastError = $null
    for ($try = 1; $try -le 5; $try++) {
        ...
        if (-not (Test-Path -LiteralPath $backupDir)) { break }
        if ($try -lt 5) { Start-Sleep -Milliseconds (200 * $try) }
    }
    if (Test-Path -LiteralPath $backupDir) {
        throw "退避 $backupDir を 5 回試しても削除できなかった。手で消すこと（残すと次回の install が誤動作する）: $lastError"
    }
} else {
